...
- **Debossed Maker's Mark:** Clean Cold Front Forge snowflake debossed flat into the underside.

## 📦 Capacity

- ~35 Standard 35pt Toploaders
...
Because this design is fully parametric, you can easily tweak pocket depths, clearances, or wall thicknesses to fit thicker supplies (like 55pt–130pt toploaders or magnetic one-touches) using the included `cold-snap-desk-caddy.scad` file.

**Key Variables You Can Change:**

- `BOX_W` - Total overall width across X (Default: `170.0`)
- `BOX_D` - Total overall depth across Y (Default: `104.0`)
- `RIGHT_BAY_W` - Width of the right-hand penny sleeve bay (Default: `70.0`)
- `LEFT_FRONT_D` - Depth of the front toploader compartment (Default: `58.0`)
- `ENABLE_LOGO` - Toggle base debossed logo rendering on/off (Default: `true`)

_To modify, open the `.scad` file in [OpenSCAD](https://openscad.org/), adjust the variables at the top of the script, press `F6` to render, and `F7` to export your new STL._
...
2. Load your supplies into their designated bays:
   - Full-depth right bay: Penny sleeves
   - Left-front bay: Standard 35pt toploaders
   - Left-rear bay: Semi-rigid submission holders
